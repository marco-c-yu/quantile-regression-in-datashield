...
> 
> 3. $w_{m,i;t}=\frac{ \tau I(y_m,i \ge X_m,i \hat\beta_{\tau;t} ) + (1- \tau ) I(y_m,i < X_m,i \hat\beta_{\tau;t} ) }{\sqrt{(y_m,i - X_m,i \hat\beta_{\tau;t})^2 + \Delta^2}}$
> 4. the weighted predictors $(W_{m,t}^{1/2}X_m)$ and weighted response $(W_{m,t}^{1/2}y_m)$
> 5. $(X_m^TW_{m,t}X_m)$ and $(X_m^TW_{m,t}y_m)$
> In server, compute
> 
> 7. $(X^TW_tX) = \sum_m (X_m^TW_{m,t}X_m)$ and $(X^TW_tX) = \sum_m (X_m^TW_{m,t}y_m)$
> 8. solve the matrix inverse $(X^TW_tX)^{-1}$
> 9. compute $\beta_{\tau;t+1} = (X^TW_tX)^{-1}X^TW_ty$
> 10. replace $t$ by $t+1$
